...
void rafgl_raster_draw_spritesheet_negative(rafgl_raster_t *raster, rafgl_spritesheet_t *spritesheet, int sheet_x, int sheet_y, int x, int y);
    ...
    for(yi = fuc+2; yi < fdc; yi++)
    {
        for(xi = flc; xi < frc; xi++)
        {
            ...
            if(sampled.rgba != RAFGL_COLOUR_KEY.rgba)
            {
                ...
                int g= 255 - sampled.g;
                int b = 255 - sampled.b;
                pixel_at_pm(raster, xi, yi).rgba = rafgl_RGBA(r, g, b, 255);
            }
        }
    }
